...
import { useGetActiveOnboardingVideoQuery } from "../../api/onBoardingApi";
...
export default function LoanApplicationModern() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("signature", form.full_name);
      ...
      // case: detail message (common DRF pattern)
      if (data.detail) {
        toast.error(data.detail);
        return;
      }

      // case: field errors (Django validation)
  ...
  const { data: activeVideo } = useGetActiveOnboardingVideoQuery("application");
  // const navigate = useNavigate();
  // =========================
  // UI
  // =========================
  return (
    <div className="bg-linear-to-br from-primary via-[#0f2a52] to-secondary">
      <Header />
      ...
      <div className="min-h-screen flex bg-linear-to-br from-primary via-[#0f2a52] to-secondary">
        {/* LEFT PANEL */}
        <div className="hidden lg:flex w-1/2 flex-col  justify-between items-start p-12 text-white">
          <div className="flex flex-col gap-y-40">
            ...
            <div className={`${activeVideo ? "my-2" : "my-20"}`}>
              <div className="absolute inset-0 opacity-20 animate-pulse pointer-events-none">
                <svg viewBox="0 0 500 300" className="w-full h-[60vh]">
                  <path
            ...
            <div>
              {activeVideo && (
                <div className="relative mb-10 rounded-2xl overflow-hidden  shadow-2xl border border-white/10">
                  ...
                  {/* OVERLAY LABEL */}
                  <div className="absolute top-2 left-2 bg-black/60 text-white text-xs px-3 py-1 rounded-full backdrop-blur">
                    🎥 Application Guide
                  </div>
                  ...
                  <div className="p-3 bg-black/40 backdrop-blur">
                    ...
                    <p className="text-xs text-gray-300">
                      Follow this guide to complete your application correctly
                    </p>
                  </div>
                </div>
              )}
        ...
        {/* FORM */}
        <div className="flex w-full lg:w-1/2  items-center justify-center px-4 py-20">
          <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl p-8 space-y-6">
            ...
            <div>
              ...
              {isFetching || (loan_typeLoading && <Loader />)}
              {loanTypes && loanTypes?.length > 0 && (
                <Select
                  ...
                  options={loanTypes
                    ...
                    .map((lt: any) => ({
                      label: `${lt.name} (range: ${formatCurrency(lt.min_amount) || 0} - ${formatCurrency(lt.max_amount)} )`,
                      value: lt.id,
                    }))}
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <Input
                  ...
                  name="signature"
                  disabled={true}
                  value={form.full_name ? `Signed by ${form.full_name}` : ""}
                  onChange={handleChange}
