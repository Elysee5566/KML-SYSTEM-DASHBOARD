...
export default function LoanTypeManagementPage() {
  const { data: loanTypesData = [], isLoading } = useGetLoanTypesQuery();
  ...
  const stats = useMemo(() => {
    ...
    const avgInterest =
      total > 0
        ? (
            loanTypes.reduce(
              (acc: number, l: any) => acc + Number(l.interest_rate),
              0,
            ) / total
          ).toFixed(1)
        : 0;
    ...
    try {
      const payload = {
        ...
        late_payment_penalty_percentage: Number(
          form.late_payment_penalty_percentage,
        ),
      ...
      <div className="space-y-4">
        {filteredLoans.length === 0 ? (
          ...
          <div>
            {/* ================= MOBILE CARDS ================= */}
            {grid && (
              <div className="space-y-4 space-x-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div className="p-5 space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-slate-700">
                            {formatCurrency(item.max_amount)}
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-slate-700">
                            {item.repayment_period_value}{" "}
                            {item.repayment_period_unit}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-slate-100 bg-slate-50 px-5 py-4">
                      <div className="flex justify-end gap-2">
                        ...
                        <Action
                          ...
                          danger
                          onClick={() => {
                            if (confirm("Delete loan type?")) {
                              deleteLoanType(item.id);
                            }
                          }}
                        />
                      </div>
            ...
            {/* ================= DESKTOP TABLE ================= */}
            {!grid && (
              <div className="w-[80vw] md:w-[76vw] lg:w-full bg-white rounded-2xl shadow overflow-x-auto">
...
const Drawer = ({
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full pb-7 w-full sm:w-135 bg-white shadow-xl transition ${open ? "translate-x-0" : "translate-x-full"}`}
    >
      ...
      <div className="p-4 space-y-4 overflow-y-auto h-[calc(100%-150px)]">
        {activeTab === "overview" && (
          <Grid>
            ...
            <Full
              ...
              disabled={viewMode}
              onChange={(v: any) => setForm({ ...form, description: v })}
            />
            <Input
              label="Min Amount"
              value={form.min_amount}
              disabled={viewMode}
              onChange={(v: any) => setForm({ ...form, min_amount: v })}
            />
            <Input
              label="Max Amount"
              value={form.max_amount}
              disabled={viewMode}
              onChange={(v: any) => setForm({ ...form, max_amount: v })}
            />
          </Grid>
        )}
        ...
        {activeTab === "financials" && (
          <Grid>
            <Input
              ...
              disabled={viewMode}
              onChange={(v: any) => setForm({ ...form, interest_rate: v })}
            />
            <Select
              label="Interest Type"
              ...
              disabled={viewMode}
              onChange={(v: any) => setForm({ ...form, interest_type: v })}
            />
            <Input
              label="Processing %"
              ...
              disabled={viewMode}
              onChange={(v: any) =>
                setForm({ ...form, processing_fee_percentage: v })
              }
            />
            <Input
              label="Penalty %"
              value={form.late_payment_penalty_percentage}
              disabled={viewMode}
              onChange={(v: any) =>
                setForm({ ...form, late_payment_penalty_percentage: v })
              }
            />
          </Grid>
        )}
        ...
        {activeTab === "rules" && (
          <Grid>
            <Input
              label="Period Value"
              value={form.repayment_period_value}
              disabled={viewMode}
              onChange={(v: any) =>
                setForm({ ...form, repayment_period_value: v })
              }
            />
            <Select
              label="Unit"
              value={form.repayment_period_unit}
              options={["days", "weeks", "months"]}
              disabled={viewMode}
              onChange={(v: any) =>
                setForm({ ...form, repayment_period_unit: v })
              }
            />
            <Select
              label="Frequency"
              ...
              disabled={viewMode}
              onChange={(v: any) =>
                setForm({ ...form, repayment_frequency: v })
              }
            />
            <Input
              label="Grace Days"
              ...
              disabled={viewMode}
              onChange={(v: any) => setForm({ ...form, grace_period_days: v })}
            />
            <Input
              label="Max Loans"
              value={form.max_concurrent_loans}
              disabled={viewMode}
              onChange={(v: any) =>
                setForm({ ...form, max_concurrent_loans: v })
              }
            />
          </Grid>
        )}
          ...
          <div className="space-y-3">
            <label>
              <input
                ...
                disabled={viewMode}
                onChange={(e) =>
                  setForm({ ...form, requires_collateral: e.target.checked })
                }
              />
              Requires Collateral
            </label>
            ...
            {form.requires_collateral && (
              <textarea
                ...
                disabled={viewMode}
                onChange={(e) =>
                  setForm({ ...form, collateral_description: e.target.value })
                }
              />
            )}
